...
- **explorer/** - file tree with Material/Catppuccin icons (`iconResolver.ts`), fuzzy search, keyboard nav, inline rename, context actions. Backslash-aware `basename`. The tree never auto-lists the home directory (`lib/rootVisibility.ts`): when the derived root is home (the fallback when nothing was chosen) the panel shows an Open Folder empty state instead; picking a folder via the native dialog cd's the terminal there and the tree follows, and picking home itself opts into showing it for the session.
